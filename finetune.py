...
class CNN():
    ...
    def train(self, X_train, y_train, X_test, y_test, NUM_CLASS):
        ...
        with custom_object_scope(
                {'ComplexInput': complex_layers.ComplexInput, 'ComplexConv1D': complex_layers.ComplexConv1D,
            ...
            m = tf.keras.models.load_model(self.trainModelPath,
                                           custom_objects={'ComplexInput': complex_layers.ComplexInput})
    ...
    def tuneTheModel(self, X_train, y_train, NUM_CLASS):
        ...
        with custom_object_scope(
                {'ComplexInput': complex_layers.ComplexInput, 
                 'ComplexConv1D': complex_layers.ComplexConv1D,
                 'ComplexAvgPooling1D': complex_layers.ComplexAvgPooling1D,
                 'ComplexFlatten': complex_layers.ComplexFlatten,
                 'ComplexDense': complex_layers.ComplexDense,
                 'ComplexDropout': complex_layers.ComplexDropout,
                 'ComplexBatchNormalization': complex_layers.ComplexBatchNormalization,
                 'ComplexAverageCrossEntropy': ComplexAverageCrossEntropy}):
            old_model = tf.keras.models.load_model(self.trainModelPath, compile=False)
    ...
    def test_close(self, new_model, X_test, y_test):
        ...
        ranks = class_ranks(new_model, X_test, y, 6, preds=None)
...
def main(opts, dataOpts):
    ...
    if 'tsn' == opts.testType:
        ...
        for sn in snList:
            ...
            rtnLine = rtnLine + 'model: {}, stride={}, trsn={}, test={}, tune model with data {}, acc is: {:f}, and std is: {:f}, to5acc is: {:f}, and std is: {:f}\n'.format(
                opts.modelType, dataOpts.stride, sn[0], sn[1], model.tuneData, mean(acc_list), stdev(acc_list), mean(acc_top5_list), stdev(acc_top5_list))
...
if __name__ == "__main__":
    ...
    opts = testOpts(trainData='/work/swanlab/fafrin2/usrp_b200/loc_1/loc_1/',
                    tuneData='/work/swanlab/fafrin2/usrp_b200/loc_4/loc_4/',
                    location='after_fft',
                    testType='tsn',
                    modelType='complex',
                    modelPath='/work/swanlab/fafrin2/final/usrp200/outlab_outdoor/after_fft/crelu/s288/res_out/modelDir/IQ_model_out_lab_complex_after_fft_slices_1000_startIdx_100000_stride_288_len_288_STFT_64.h5')
    # if opts.useGpu:
    #     os.environ['CUDA_VISIBLE_DEVICES'] = "0"
    dataOpts = load_slice_IQ.loadDataOpts(opts.tuneData, opts.location, num_slice=1000, slice_len=288,
                                          start_idx=100000, stride=288, mul_trans=True, window=64, dataType='IQ')

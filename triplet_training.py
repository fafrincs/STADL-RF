...
def runTrain(opts, sampleNumLimit=25):
    ...
    print(description)
    print("with parameters, Alpha: %s, Batch_size: %s, Embedded_size: %s, Epoch_num: %s, sampleNumLimit: %s" % (
    alpha, batch_size, emb_size, number_epoch, sampleNumLimit))
    ...
    input_shape = (opts.slice_len, 1)
    print('input shape is: ', input_shape)
    if opts.modelType == 'complex':
        shared_conv2 = rf_models.create_model(opts.modelType, inp_shape=input_shape, NUM_CLASS=6, emb_size=emb_size, classification=True)
       # shared_conv2 = rf_models.create_model(input_shape, emb_size=emb_size)
    else:
        raise
    shared_conv2.summary()
    anchor = Input(batch_shape=(None, opts.slice_len, 1), name='anchor')
    positive = Input(batch_shape=(None, opts.slice_len, 1), name='positive')
    negative = Input(batch_shape=(None, opts.slice_len, 1), name='negative')
    ...
    start = time.time()
    gen_hard = SemiHardTripletGenerator(Xa_train, Xp_train, batch_size, all_traces, all_traces_train_idx, Id2Label,
                                        None)
    ...
    modelPath = os.path.join(modelDir, 'complex_triplet_{}_{}_{}.h5'.format(source, sampleNumLimit, opts.sample))
...
def runTest(opts, modelPath, sampleLimit, trainTime, n_shot, max_n):
    ...
    rtnLine = rtnLine + '\nmodel type: {}, train sample num is: {}, sampled: {}'.format(opts.modelType, sampleLimit,
                                                                                        opts.sample)
...
class tfOpts():
    def __init__(self, source_path, test_path, location, modelType, file_key='*.bin', num_slice=1000, start_ix=0,
                 slice_len=288, stride=288, sampleData=True, mul_trans=True, window=64, dataType="IQ"):
...
if __name__ == "__main__":
    ...
    for n in [400, 350, 300, 250]:
        opts = tfOpts(source_path="/work/swanlab/fafrin2/usrp_b200/out_lab/out_lab/",
                      test_path="/work/swanlab/fafrin2/usrp_b200/out_door/out_door/",
                      modelType='complex', location='after_fft', num_slice=n, sampleData=False, mul_trans=True)
        if opts.useGpu:
        ...
        if 'tsn' == opts.testType:
            ...
            f = open(outfile, 'a+')
            print('\n\n##################### test time is: {}, sample: {}, mul_trans: {}####################'.format(
                time.ctime(), opts.sample, opts.mul_trans), file=f, flush=True)
            modelPath, time_last = runTrain(opts, sampleNumLimit=opts.num_slice)

            tsnList = tsnList[::-1]
            for tsn in tsnList:
                # opts.start_ix = opts.num_slice
                rtnLine = runTest(opts, modelPath, sampleLimit=opts.num_slice, trainTime=time_last, n_shot=tsn,
                                  max_n=tsnList[-1])

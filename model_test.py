...
def model_test():
    ...
    print('\n\n#################### My Dataset,  test time is: {} #########################'.format(time.ctime()),
          file=f)
    models = os.listdir('/lustre/work/swanlab/fafrin2/usrp200/outlab_outdoor/after_fft/crelu/s288/res_out/modelDir/')
    best_model_name = ""
    best_model_acc = 0.0
    for m in models:
        modelPath = '/lustre/work/swanlab/fafrin2/usrp200/outlab_outdoor/after_fft/crelu/s288/res_out/modelDir/' + m
        ...
        m.summary()
        dataPath = "/work/swanlab/fafrin2/usrp_b200/out_door/"
        for c in comb:
            acc_list = list()
            for i in range(test_times):
                opts = tfOpts(source_path=dataPath + "out_door/", location='after_fft')

...
def create_test_set_Wang_disjoint(signature_dict, test_dict, sites, features_model, type_exp):
    ...
    for i in sites:
        ...
        signature_instance = signature_instance.astype('complex64')
    ...
    for i in sites:
        ...
        test_instance = test_instance.astype('complex64')
...
def computeTopN(topN, knn, X_test, y_test):
    ...
    acc_knn_top3 = float(count_correct) / float(len(X_test))
    acc_knn_top3 = float("{0:.15f}".format(round(acc_knn_top3, 6)))
...
def kNN_train(signature_vector_dict, params):
    ...
    knn = KNeighborsClassifier(n_neighbors=3, weights=params['weights'], p=params['p'], metric=params['metric'], algorithm='brute')
...
def kNN_accuracy(signature_vector_dict, test_vector_dict, params):
    ...
    # Top-5
    acc_knn_top3 = computeTopN(3, knnModel, X_test, y_test)
    print('KNN accuracy Top1 = ', acc_knn_top1, '\tKNN accuracy Top3 = ', acc_knn_top3)
    ranks = ranks_KNN(y_test, knnModel.predict(X_test), 6)
    df = pd.DataFrame(ranks, index=None)

    df.to_csv('ADA_KNN_N_'+ str(params['k']) + '.csv', header=False)
    return acc_knn_top1, acc_knn_top3
